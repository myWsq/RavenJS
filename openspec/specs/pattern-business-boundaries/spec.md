# pattern-business-boundaries Specification

## Purpose

定义 RavenJS pattern 文档中的业务边界语言，明确区分 `transport validation`、`domain invariants` 与 `persistence constraints`，并要求所有入口无关的业务逻辑收敛到 entity。
...
- **WHEN** Agent 阅读 `reference/overview.md` 或 `reference/layer-responsibilities.md` 中关于 `Interface Unit` 的说明
...
- **WHEN** Agent 快速浏览 `reference/*` 中与业务代码分层相关的文档
