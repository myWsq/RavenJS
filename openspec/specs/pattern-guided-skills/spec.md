...
RavenJS 的 raven-use 技能 SHALL 在 Agent 处理 RavenJS 业务代码或运行时装配任务时，显式要求其读取 `pattern/*` 中与当前任务相关的文档，而不能只停留在 API、README 或源码浏览层面。
...
- **THEN** 技能 SHALL 指导 Agent 先读取 `pattern/overview.md`
...
- **THEN** 技能 SHALL 指导 Agent 读取 `pattern/runtime-assembly.md`
...
RavenJS 的 raven-use 技能 SHALL 在 Agent 准备结束任务前，要求其依据 `pattern/anti-patterns.md` 与 `pattern/conventions.md` 对本次产出执行显式自检。
...
- **THEN** 技能 SHALL 要求其检查文件放置位置与命名是否符合 `pattern/conventions.md`
...
- **THEN** 技能 SHALL 指导 Agent 进入 `pattern/runtime-assembly.md` 的学习路径
