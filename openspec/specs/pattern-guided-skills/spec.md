...
RavenJS 的 raven-use 技能 SHALL 在 Agent 处理 RavenJS 业务代码或运行时装配任务时，显式要求其读取技能自带的 pattern reference（`reference/*`，随 raven-use 技能一起分发，而非随 `@raven.js/core` 包发布）中与当前任务相关的文档，而不能只停留在 API、README 或源码浏览层面。
...
- **WHEN** Agent 使用 `raven-use` 处理 route、handler、entity、repository、command、query、dto 或查询结果映射等业务代码任务
- **THEN** 技能 SHALL 指导 Agent 先读取 `reference/overview.md`
...
- **THEN** 技能 SHALL 指导 Agent 读取 `reference/runtime-assembly.md`
...
RavenJS 的 raven-use 技能 SHALL 在 Agent 开始修改 RavenJS 业务代码前，要求其先完成一个简短的 Pattern Plan，对任务类型、所需层级和目标文件布局做出明确判断。对于读路径建模，Pattern Plan SHALL 使用 `DTO`、查询结果映射或 `complex read` 等更稳定的语言，而 MUST NOT 默认把 `Projection` 作为推荐扩展层。
...
RavenJS 的 raven-use 技能 SHALL 在 Agent 准备结束任务前，要求其依据 `reference/anti-patterns.md` 与 `reference/conventions.md` 对本次产出执行显式自检。
...
- **THEN** 技能 SHALL 要求其检查文件放置位置与命名是否符合 `reference/conventions.md`
- **AND** SHALL 对任何刻意的命名或目录偏离进行显式说明

### Requirement: 技能在 Pattern Plan 中必须判断 dependency placement

RavenJS 的 raven-use 技能 SHALL 在 Agent 规划可复用依赖时，显式判断该依赖属于 runtime state 还是 `Object Style Service`。技能 MUST NOT 因其需要复用就默认写成 class 或引入 `AppState`。若依赖不需要由 Raven runtime 管理初始化、生命周期或作用域，技能 SHALL 优先保持 plain object / function collection 的直接导出模块。
...
- **THEN** 技能 SHALL 指导 Agent 进入 `reference/runtime-assembly.md` 的学习路径
